...
public void configure (ClientDetailsServiceConfigurer clients) throws Exception {
		 ...
		 * .autoApprove (true) .secret (passwordEncoder (). encode ("{noop}password"));
		 */

clients.inMemory().withClient("client").secret("{noop}password").scopes("read", "write")
.authorizedGrantTypes("password", "authorization_code", "refresh_token", "implicit").accessTokenValiditySeconds(100)
